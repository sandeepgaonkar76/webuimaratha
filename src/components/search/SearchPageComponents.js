...
class SearchPageComponents extends Component{
	...
	handlequicksearch(e)
	{	e.preventDefault();
	const {ageFrom,ageTo,gender}=this.state;
	...
	const requestOptions = {
		...
		body: JSON.stringify({
			...
			caste:parseInt(this.state.caste),
		})
	};
...
validate = (e) =>{
    ...
    if (!ageTo) {
		...
		{
			isValid = false;
			errors["ageFrom"] = "Invalid age From .";
		}
		this.setState({
		  errors: errors
		});	
...
componentDidMount()
{
	...
	this.getAllHeight();
	this.getAllCaste();
	this.setState({Mplan:localStorage.getItem('Mplan',data.planID)});   	
	
}
redirectToSearchResult = (e) => {
    // e.preventDefault();    
    const { history } = this.props;
    history.push('/search-result',this.state);
    }
    render() {
		const { history } = this.props;
		const { ageFrom,ageTo,gender,searchresult,heightFrom,heightTo,castev} = (this.props.location && this.props.location.state) || {};
        return (
        <div>
        <HeaderComponents/>
...
<section className="search text-center">
  ...
  <div className="headinf-tab">
                              ...
                              <option value="0" defaultChecked>Select Caste</option>
                                {
                                  ...
                                  return <option key={obj.castID} value={obj.castID}>{obj.castName}</option>
										...
										<div className="row">
											<div className="col-md-12">
												...
												<button type="submit" className="btn btn-primary" name="search_now">Search Now</button>
												 </div>
											 </div>
  ...
  <Tab eventKey="2a" title="Advanced Search" style={{display:this.state.Mplan==1?"none":"block"}}>
                                  ...
                                  return (<option key={obj.castID} value={obj.castID}>{obj.castName}</option>)
										...
										<div className="row">
											<div className="col-md-12">
												...
												<button type="submit" className="submit1 btn">Search Now</button>
												 </div>
											 </div>
