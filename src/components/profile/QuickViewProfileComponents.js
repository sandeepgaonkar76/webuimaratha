...
// fetch(`${process.env.REACT_APP_API_KEY}Member/12`)
//   .then(function(res) {
//       return res.json();
//   }).then((json)=> {
// 	  console.log(json)
...
componentDidMount(){}
    render() {
        ...
        const { ageFrom,ageTo,gender,searchresult,quickviewprofileid} = (this.props.location && this.props.location.state) || {};
		const popover = (
      ...
      <button type="button" className="btn btn-primary submit-btn1"  onClick={this.handleShow}>View Profile</button>
			
			
			<Modal  show={this.state.show} fullscreen={true} onHide={this.handleClose} id="myModal" className="showprofile">
			  
			   <Modal.Header closeButton>
          <Modal.Title><h2 className="exampleModalCenterh2">{this.state.profiledetails.firstName} {this.state.profiledetails.middleName} {this.state.profiledetails.lastName}</h2></Modal.Title>
        </Modal.Header>
			  <Modal.Body>
			...
			<div>
				<div className="row">
			  		<div className="col-2"><img src={this.state.profiledetails.s3ImagePath} title="Test2 Test" alt="IN14" className="img-responsive gtFullWidth" style={{height:"150px",borderRadius:"150px"}} /></div>
					<div className="col">	
